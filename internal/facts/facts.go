...
Прежде чем отвечать, проверь карточку: есть ли в ней отдельный ключ для предмета разговора — того, о чём он вообще идёт? Если предмет известен, а такого ключа нет, заведи его этой же правкой — даже когда новое сообщение совсем о другом. Предмет, спрятанный внутри «цели» или «задачи», считается отсутствующим: цель по ходу разбора переписывается, и предмет ушёл бы вместе с ней.

Ключ называй самим предметом — «машина», «система», «продукт», «документ», «дом»; слово «предмет» ключом не бери. Значение — только сам предмет с его опознавательными признаками (марка, модель, год, название, версия), без цели, без работ и без сроков: для них свои ключи.
